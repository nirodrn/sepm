...
const PackingMaterialSupplierAllocation = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const allocationData = {
        ...
        allocations: allocations.filter(a => a.supplierId && a.quantity).map(allocation => ({
      ...
      await packingMaterialsService.createSupplierAllocation(allocationData);
  ...
  const requestedQuantity = requestData?.items?.[0]?.quantity || 0;
  const isFullyAllocated = getTotalAllocated() === requestedQuantity;
  const isOverAllocated = getTotalAllocated() > requestedQuantity;
    ...
    <div className="p-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center">
              ...
              Packing Material Supplier Allocation
            </h1>
            <p className="text-gray-600 mt-2">Allocate approved packing material request across suppliers</p>
          </div>
        </div>
      </div>
      ...
      {requestData && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          {requestData.budgetEstimate && (
            <p className="text-blue-700 text-sm">
              Budget Estimate: ${requestData.budgetEstimate.toFixed(2)}
            </p>
          )}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="max-w-6xl">
          ...
          <div className="mt-6 bg-gray-50 rounded-lg p-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <span className={`text-lg font-semibold ml-2 ${
                  ...
                  {getTotalAllocated()} / {requestedQuantity} {requestData?.items?.[0]?.unit}
                </span>
              </div>
            ...
            <div className="mt-2">
              <span className={`text-sm font-medium ${
                isFullyAllocated && !isOverAllocated ? 'text-green-600' :
                isOverAllocated ? 'text-red-600' :
                'text-yellow-600'
              }`}>
                {isFullyAllocated && !isOverAllocated ? 'Fully Allocated' :
        ...
        <div className="flex items-center justify-end space-x-4">
          ...
          <button
            type="submit"
            disabled={loading || !isFullyAllocated || isOverAllocated}
