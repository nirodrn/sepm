...
import { ShoppingCart, Search, Filter, Eye, CheckCircle, TruckIcon, Package, AlertTriangle } from 'lucide-react';
import { packingMaterialRequestService } from '../../../services/packingMaterialRequestService';
import { supplierService } from '../../../services/supplierService';
import LoadingSpinner from '../../../components/Common/LoadingSpinner';
import { formatDate } from '../../../utils/formatDate';

const PackingMaterialRequestList = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [requestData, supplierData] = await Promise.all([
        packingMaterialRequestService.getPackingMaterialRequests(),
  ...
  const handleMarkAsReceived = async (requestId) => {
    try {
      await packingMaterialRequestService.markAsReceived(requestId);
      await loadData();
    } catch (error) {
      setError(error.message);
    }
  ...
  const handleAddToStore = async (requestId) => {
    try {
      await packingMaterialRequestService.addToStore(requestId);
      await loadData();
    } catch (error) {
      setError(error.message);
    }
  ...
  if (loading) {
    return <LoadingSpinner text="Loading packing material requests..." />;
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">Manage packing material purchase requests and deliveries</p>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-start space-x-4 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="space-y-1">
                      {request.materials?.slice(0, 3).map((material, index) => (
                        <div key={index} className="text-sm text-gray-600">
                          • {material.materialName}: {material.requestedQuantity} {material.unit}
                        </div>
                      ))}
                ...
                <div className="flex items-center space-x-2 ml-4">
                  {request.status === 'md_approved' && (
                    <button
                      onClick={() => navigate(`/warehouse/packing-materials/requests/${request.id}/allocate`)}
                  ...
                  <button
                    onClick={() => navigate(`/warehouse/packing-materials/requests/${request.id}`)}
...
export default PackingMaterialRequestList;