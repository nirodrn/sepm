...
const SupplierAllocation = () => {
  ...
  const loadData = async () => {
    try {
      ...
      if (request.requestedBy) {
        try {
          const userData = await userService.getById(request.requestedBy);
  ...
  const isValidAllocation = () => {
    if (allocations.length === 0) return true; // Allow empty state
    
    // Only check for over-allocation - disable if any material is over-allocated
    return allocations.every(material => {
      ...
      return !isOverAllocated; // Return false only if over-allocated
    });
  };
  ...
  const generateSupplierPDF = (supplierData, allocationData) => {
    ...
    doc.text(`Priority: ${requestData?.priority || 'Normal'}`, margin + 5, 123);
    ...
    // Authorization Section (Single signature space)
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    ...
    doc.rect(pageWidth - 75, yPosition + 22, 30, 15); // Seal box
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <button
                  onClick={() => navigate('/warehouse/raw-materials/requests')}
                ...
                <h1 className="text-2xl font-bold text-gray-900">Supplier Allocation</h1>
              </div>
              <button
