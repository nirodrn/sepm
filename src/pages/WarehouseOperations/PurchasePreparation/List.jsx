...
import { ShoppingCart, Search, Filter, Eye, TruckIcon, Package, CheckCircle, Clock } from 'lucide-react';
...
const PurchasePreparationList = () => {
  ...
  const handleAssignSupplier = (preparationId) => {
    navigate(`/warehouse/purchase-preparation/${preparationId}/assign-supplier`);
  };

  const handleMarkDelivered = (preparationId) => {
    navigate(`/warehouse/purchase-preparation/${preparationId}/mark-delivered`);
  };
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
        ...
        <div className="bg-blue-50 rounded-lg p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-blue-600">Supplier Assigned</p>
                ...
                <tr key={prep.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex items-center justify-end space-x-2">
                      {prep.status === 'pending_supplier_assignment' && (
                        <button
                          onClick={() => handleAssignSupplier(prep.id)}
                          className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm transition-colors"
                        >
                          Assign Supplier
                        </button>
                      )}
                      {prep.status === 'supplier_assigned' && (
                        <button
                          onClick={() => handleMarkDelivered(prep.id)}
